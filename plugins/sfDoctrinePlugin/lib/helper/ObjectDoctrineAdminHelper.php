 ...
 * @version    SVN: $Id: ObjectDoctrineAdminHelper.php 8688 2008-04-30 03:05:24Z Jonathan.Wage $
 */
...
function object_doctrine_admin_double_list($object, $method, $options = array())
{
  ...
  $callback = $callback ? $callback:'_get_doctrine_object_list';
...
function object_doctrine_admin_select_list($object, $method, $options = array())
{
  $callback = _get_option($options, 'callback');
  $callback = $callback ? $callback:'_get_doctrine_object_list';
  return object_admin_select_list($object, $method, $options, $callback);
}
