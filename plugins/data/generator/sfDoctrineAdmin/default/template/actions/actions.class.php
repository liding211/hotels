 ...
 * @version    SVN: $Id: actions.class.php 7736 2008-03-04 16:37:59Z Jonathan.Wage $
 */
...
<?php endif; ?>
  ...
  protected function addSortCriteria ($q)
  {
    if ($sort_column = $this->getUser()->getAttribute('sort', null, 'sf_admin/<?php echo $this->getSingularName() ?>/sort'))
    {
      ...
      $columns = array('subject', 'name', 'title', 'description', 'id');
      if ( ! $table->hasColumn($sort_column) && $table->hasRelation($sort_column))
      {
        $relation = $table->getRelation($sort_column);
        foreach ($columns as $column)
        {
          if ($relation->getTable()->hasColumn($column))
          {
            $sort_column = $sort_column . '.' . $column;
            break;
          }
        }
      }
